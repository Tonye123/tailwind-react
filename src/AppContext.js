...
import useLocal from './useLocal';
 ...
 //creates a context value that can be shared across our application
...
const appStateReducer = ( state, action ) => {
    switch (action.type) {
        case "READ_BOOK": {
            ...
            return { ...state, toRead: newToRead,
             completed: newCompleted}
        }
        case "UNREAD_BOOK": {
            return {...state, toRead:state.toRead.concat(action.item), 
...
export function AppStateProvider({ children }) {
    let bookState = useReducer(appStateReducer, initialState)
    //useLocal();
    return (
    <AppContext.Provider value={bookState}>{children}</AppContext.Provider>
    )
